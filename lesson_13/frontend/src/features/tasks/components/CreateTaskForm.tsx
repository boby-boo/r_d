  ...
  return (
    <form id="task-form" className="task-form" onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="input-field">
        ...
        <select
          className={`status ${errors.status ? 'error' : ''}`}
          id="status"
          {...register('status')}
        >
      ...
      <div className="input-field">
        ...
        <select
          className={`priority ${errors.priority ? 'error' : ''}`}
          id="priority"
          {...register('priority')}
        >
      ...
      <div className="input-field">
        ...
        <input
          type="date"
          id="deadline"
          className={`deadline ${errors.deadline ? 'error' : ''}`}
          {...register('deadline')}
        />
        {errors.deadline && <p className="error-text">{errors.deadline.message}</p>}
      </div>
      <button data-action="submit-button" className="btn-primary" disabled={!isValid}>
        {' '}
        {task ? 'Update Task' : 'Add Task'}
      </button>
      <button type="button" data-action="reset-button" className="btn-secondary" onClick={onReset}>
        Reset Form
      </button>
    </form>
  );
